...
if (empty($_GET['search'])) {
    $statement = $pdo->prepare("SELECT * FROM tbl_nhanhieu");
} else {
    $search = $_GET['search'];
    if(is_numeric($search))
    {
        $statement = $pdo->prepare("SELECT * FROM tbl_nhanhieu where id_nh regexp $search or nhanhieu regexp $search ");
    }
    else{
        $statement = $pdo->prepare("SELECT * FROM tbl_nhanhieu where nhanhieu regexp '$search'");
    }
}
